...
public class WebAuthComponent {
    ...
    private static var callbacks = [String: (Result<String, RevenexxAPIRevenexxError>) -> Void]()
    #if canImport(AuthenticationServices)
    ...
    internal static func authenticate(
        ...
        onComplete: @escaping (Result<String, RevenexxAPIRevenexxError>) -> Void
    ) {
    ...
    private static func cleanUp() {
        callbacks.forEach { (_, callback) in
            callback(.failure(RevenexxAPIRevenexxError(message: "User cancelled login.")))
        }
