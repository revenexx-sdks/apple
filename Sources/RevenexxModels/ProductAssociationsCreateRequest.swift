...
/// 
open class ProductAssociationsCreateRequest: Codable {
    ...
    public required init(from decoder: Decoder) throws {
        ...
        self.association_type_id = try container.decode(String.self, forKey: .association_type_id)
        self.position = try container.decodeIfPresent(Int.self, forKey: .position)
        self.product_id = try container.decode(String.self, forKey: .product_id)
        self.quantity = try container.decodeIfPresent(Double.self, forKey: .quantity)
        self.target_product_id = try container.decode(String.self, forKey: .target_product_id)
    }

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)

        try container.encode(association_type_id, forKey: .association_type_id)
        try container.encodeIfPresent(position, forKey: .position)
        try container.encode(product_id, forKey: .product_id)
        try container.encodeIfPresent(quantity, forKey: .quantity)
        try container.encode(target_product_id, forKey: .target_product_id)
    }
    ...
    public static func from(map: [String: Any] ) -> ProductAssociationsCreateRequest {
