...
/// 
open class CartVocabularyRef: Codable {
    ...
    public let name: RevenexxEnums.CartVocabularyRefName?
    /// A plain string, or a locale map keyed by language tag ({"en": …, "de": …}). Read the requested tag, fall back to `en`.
    public let title: [String: AnyCodable]?

    init(
        description: [String: AnyCodable]?,
        name: RevenexxEnums.CartVocabularyRefName?,
    ...
    public required init(from decoder: Decoder) throws {
        ...
        if let nameString = try container.decodeIfPresent(String.self, forKey: .name) {
            self.name = RevenexxEnums.CartVocabularyRefName(rawValue: nameString)
        } else {
            self.name = nil
        }
    ...
    public static func from(map: [String: Any] ) -> CartVocabularyRef {
        return CartVocabularyRef(
            description: map["description"] as? [String: AnyCodable],
            name: map["name"] as? String != nil ? CartVocabularyRefName(rawValue: map["name"] as! String) : nil,
