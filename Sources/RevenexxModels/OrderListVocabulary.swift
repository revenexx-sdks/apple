...
/// 
open class OrderListVocabulary: Codable {
    ...
    public let default_tone: RevenexxEnums.OrderListVocabularyDefaultTone?
    /// A plain string, or a locale map keyed by language tag ({"en": …, "de": …}). Read the requested tag, fall back to `en`.
    public let description: [String: AnyCodable]?
    /// Vocabulary name, unique within the app.
    public let name: RevenexxEnums.OrderListVocabularyName?
    /// 'schema' — a CHECK constraint owns the set; 'table' — the tenant's own rows do.
    public let source: RevenexxEnums.OrderListVocabularySource?
    /// A plain string, or a locale map keyed by language tag ({"en": …, "de": …}). Read the requested tag, fall back to `en`.
    public let title: [String: AnyCodable]?
    /// Every permitted value, in the order a select should offer them.
    public let values: [OrderListVocabularyValue]?

    init(
        app: String?,
        closed: Bool?,
        default_tone: RevenexxEnums.OrderListVocabularyDefaultTone?,
        description: [String: AnyCodable]?,
        name: RevenexxEnums.OrderListVocabularyName?,
        source: RevenexxEnums.OrderListVocabularySource?,
    ...
    public required init(from decoder: Decoder) throws {
        ...
        if let default_toneString = try container.decodeIfPresent(String.self, forKey: .default_tone) {
            self.default_tone = RevenexxEnums.OrderListVocabularyDefaultTone(rawValue: default_toneString)
        } else {
        ...
        if let nameString = try container.decodeIfPresent(String.self, forKey: .name) {
            self.name = RevenexxEnums.OrderListVocabularyName(rawValue: nameString)
        } else {
            self.name = nil
        }
        if let sourceString = try container.decodeIfPresent(String.self, forKey: .source) {
            self.source = RevenexxEnums.OrderListVocabularySource(rawValue: sourceString)
        } else {
            self.source = nil
        }
        self.title = try container.decodeIfPresent([String: AnyCodable].self, forKey: .title)
        self.values = try container.decodeIfPresent([OrderListVocabularyValue].self, forKey: .values)
    }
    ...
    public static func from(map: [String: Any] ) -> OrderListVocabulary {
        return OrderListVocabulary(
            app: map["app"] as? String,
            closed: map["closed"] as? Bool,
            default_tone: map["default_tone"] as? String != nil ? OrderListVocabularyDefaultTone(rawValue: map["default_tone"] as! String) : nil,
            description: map["description"] as? [String: AnyCodable],
            name: map["name"] as? String != nil ? OrderListVocabularyName(rawValue: map["name"] as! String) : nil,
            source: map["source"] as? String != nil ? OrderListVocabularySource(rawValue: map["source"] as! String) : nil,
            title: map["title"] as? [String: AnyCodable],
            values: (map["values"] as? [[String: Any]] ?? []).map { OrderListVocabularyValue.from(map: $0) }
        )
    }
}
