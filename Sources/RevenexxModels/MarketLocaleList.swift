...
open class MarketLocaleList: Codable {
    ...
    public required init(from decoder: Decoder) throws {
        ...
        self.filter = try container.decodeIfPresent(MarketLocaleFilter.self, forKey: .filter)
        self.items = try container.decodeIfPresent([MarketLocale].self, forKey: .items)
    ...
    public static func from(map: [String: Any] ) -> MarketLocaleList {
        return MarketLocaleList(
            filter: MarketLocaleFilter.from(map: map["filter"] as! [String: Any]),
            items: (map["items"] as? [[String: Any]] ?? []).map { MarketLocale.from(map: $0) },
