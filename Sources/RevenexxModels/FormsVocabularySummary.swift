...
/// 
open class FormsVocabularySummary: Codable {
    ...
    public let name: RevenexxEnums.FormsVocabularySummaryName?
    /// A plain string, or a locale map keyed by language tag ({"en": …, "de": …}). Read the requested tag, fall back to `en`.
    public let title: [String: AnyCodable]?

    init(
        description: [String: AnyCodable]?,
        name: RevenexxEnums.FormsVocabularySummaryName?,
    ...
    public required init(from decoder: Decoder) throws {
        ...
        if let nameString = try container.decodeIfPresent(String.self, forKey: .name) {
            self.name = RevenexxEnums.FormsVocabularySummaryName(rawValue: nameString)
        } else {
            self.name = nil
        }
    ...
    public static func from(map: [String: Any] ) -> FormsVocabularySummary {
        return FormsVocabularySummary(
            description: map["description"] as? [String: AnyCodable],
            name: map["name"] as? String != nil ? FormsVocabularySummaryName(rawValue: map["name"] as! String) : nil,
