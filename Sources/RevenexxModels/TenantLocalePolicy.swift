...
open class TenantLocalePolicy: Codable {
    ...
    public let locales: [TenantLocaleKeys]?
    ...
    public required init(from decoder: Decoder) throws {
        ...
        self.locales = try container.decodeIfPresent([TenantLocaleKeys].self, forKey: .locales)
    }
    ...
    public static func from(map: [String: Any] ) -> TenantLocalePolicy {
        return TenantLocalePolicy(
            ...
            locales: (map["locales"] as? [[String: Any]] ?? []).map { TenantLocaleKeys.from(map: $0) }
        )
    }
}
