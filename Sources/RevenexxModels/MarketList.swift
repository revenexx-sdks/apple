...
open class MarketList: Codable {
    ...
    public required init(from decoder: Decoder) throws {
        ...
        self.filter = try container.decodeIfPresent(MarketFilter.self, forKey: .filter)
        self.items = try container.decodeIfPresent([Market].self, forKey: .items)
    ...
    public static func from(map: [String: Any] ) -> MarketList {
        return MarketList(
            filter: MarketFilter.from(map: map["filter"] as! [String: Any]),
            items: (map["items"] as? [[String: Any]] ?? []).map { Market.from(map: $0) },
