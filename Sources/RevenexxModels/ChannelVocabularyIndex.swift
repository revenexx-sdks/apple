...
/// 
open class ChannelVocabularyIndex: Codable {
    ...
    public let vocabularies: [ChannelVocabularyRef]?
    ...
    public required init(from decoder: Decoder) throws {
        ...
        self.vocabularies = try container.decodeIfPresent([ChannelVocabularyRef].self, forKey: .vocabularies)
    }
    ...
    public static func from(map: [String: Any] ) -> ChannelVocabularyIndex {
        return ChannelVocabularyIndex(
            app: map["app"] as? String,
            vocabularies: (map["vocabularies"] as? [[String: Any]] ?? []).map { ChannelVocabularyRef.from(map: $0) }
        )
    }
}
