...
open class MarketTaxClassList: Codable {
    ...
    public required init(from decoder: Decoder) throws {
        ...
        self.filter = try container.decodeIfPresent(MarketTaxClassFilter.self, forKey: .filter)
        self.items = try container.decodeIfPresent([MarketTaxClass].self, forKey: .items)
    ...
    public static func from(map: [String: Any] ) -> MarketTaxClassList {
        return MarketTaxClassList(
            filter: MarketTaxClassFilter.from(map: map["filter"] as! [String: Any]),
            items: (map["items"] as? [[String: Any]] ?? []).map { MarketTaxClass.from(map: $0) },
