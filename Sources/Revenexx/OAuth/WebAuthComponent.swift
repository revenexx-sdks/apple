...
public class WebAuthComponent {
    ...
    private static var callbacks = [String: (Result<String, RevenexxError>) -> Void]()
    #if canImport(AuthenticationServices)
    ...
    internal static func authenticate(
        ...
        onComplete: @escaping (Result<String, RevenexxError>) -> Void
    ) {
    ...
    private static func cleanUp() {
        callbacks.forEach { (_, callback) in
            callback(.failure(RevenexxError(message: "User cancelled login.")))
        }
