...
@_exported import RevenexxModels
...
open class Client {
    ...
    open var endPoint = "https://api.revenexx.com"

    open var endPointRealtime: String? = nil

    open var headers: [String: String] = [
        "content-type": "application/json",
        "x-sdk-name": "Revenexx Apple",
        "x-sdk-platform": "",
    ...
    /// @return Client
    ///
    open func setApiKeyAuth(_ value: String) -> Client {
        config["apikeyauth"] = value
        _ = addHeader(key: "X-Revenexx-Api-Key", value: value)
        return self
    }
    ...
    /// @return Client
    ///
    open func setBearerAuth(_ value: String) -> Client {
        config["bearerauth"] = value
        _ = addHeader(key: "Authorization", value: value.lowercased().hasPrefix("bearer ") ? value : "Bearer \(value)")
        return self
    }
    ...
    ///
    /// Set the market slug requests are scoped to, sent as the
    /// X-Revenexx-Market header. Optional - omit it to see only global rows.
    ///
    /// @param String value
    /// @return Client
    ///
    open func setMarket(_ value: String) -> Client {
        _ = addHeader(key: "X-Revenexx-Market", value: value)
        return self
    }
   ...
   open func parametersToQueryString(params: [String: Any?]) -> String {
    ...
    ///
    /// Sends a "ping" request to the API to verify connectivity.
    ///
    /// @return String
    /// @throws Exception
    ///
   open func ping() async throws -> String {
    ...
    private func execute<T>(
        ...
        if let warning = response.headers["x-revenexx-warning"].first {
    ...
    private func addOriginHeader() {
        ...
        _ = addHeader(
            key: "origin",
            value: "revenexx-\(operatingSystem)://\(packageInfo.packageName)"
        )
    }
}
